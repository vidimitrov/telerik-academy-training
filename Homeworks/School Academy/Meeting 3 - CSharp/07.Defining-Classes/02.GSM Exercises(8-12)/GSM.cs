...
namespace _01.GSM
{
    public class GSM
    {
        ...
        public static void AddCall(string number, int duration)
        {            
            string date = Date();
            string time = Time();
            Call call = new Call(date, time, number, duration);
        ...
        public static double TotalPrice(double pricePerMinute)
        {
            double totalPrice = 0;
            foreach (Call call in callHistory)
            {
                totalPrice += (call.Duration / 60) * pricePerMinute;
            }
            return totalPrice;
        }
